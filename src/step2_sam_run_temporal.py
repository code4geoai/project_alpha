...
from src.config import (
    RANKED_PROMPTS_DIR, PRED_MASK_DIR, PRED_VIS_DIR,
    SUMMARY_CSV
)
...
# MAIN EXECUTION
# --------------------------------------------------
def run_sam_with_temporal_prompts(dataset):
    ...
    for item in dataset:
        ...
        rp_path = os.path.join(RANKED_PROMPTS_DIR, f"ranked_prompts_{image_id}.npy")
        ...
        np.save(os.path.join(PRED_MASK_DIR, f"pred_mask_temporal_{image_id}.npy"),
        ...
        fig.savefig(os.path.join(PRED_VIS_DIR, f"vis_temporal_{image_id}.png"), dpi=150)
        plt.close(fig)
        ...
        print(f"  -> Temporal prompts={len(points_xy)}, IoU={best_iou:.3f}, F1={f1:.3f}")

        rows.append({
            "image_id": image_id,
            "n_prompts_temporal": len(points_xy),
    ...
    df.to_csv(SUMMARY_CSV.replace(".csv", "_temporal.csv"), index=False)

    print("\nSaved summary:", SUMMARY_CSV.replace(".csv", "_temporal.csv"))
    print(df)
